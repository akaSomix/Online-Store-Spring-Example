...
public interface ProdottoRepository extends JpaRepository<ProdottoEntity, Long> {
	...
	@Query("FROM ProdottoEntity p WHERE p.nome LIKE %:prefix%")
	public List<ProdottoEntity> findByNomeContainingPrefix(@Param("prefix")final String prefix);
...
//	public List<ProdottoEntity>  findAllOrderByPrezzo();
	...
	@Query("SELECT d.negozio FROM DisponibilitaEntity d JOIN d.prodotto p WHERE p.prodottoId = :prodotto_id and d.disponibile = 1")
	...
	@Query("FROM ProdottoEntity p WHERE p NOT IN"
			+ " (SELECT d.prodotto FROM DisponibilitaEntity d JOIN d.negozio n WHERE n.negozioId = :negozio_id AND d.disponibile = 1)")
